...
impl FeeCurrencyRegistry {
    ...
    /// Get CALL/USD price — fallback only (reads from EVM in caller).
    pub fn get_call_price(&self) -> u128 {
        2_000_000 // Fallback: $2.00 with 6 decimals
    }

    /// Get the USD price for any asset — fallback only (reads from EVM in caller).
    pub fn get_asset_usd_price(&self, _asset_id: AssetId) -> u128 {
        1_000_000 // Fallback: $1.00 with 6 decimals (assume stablecoin)
    }
    ...
    pub fn priority_score(
        ...
        if is_call {
            ...
            let stablecoin_usd = self.get_asset_usd_price(asset_id);
            let call_usd = self.get_call_price();
...
mod tests {
    ...
    fn test_priority_score_call() {
        ...
        let score = registry.priority_score(1_000_000, true, 1);
    ...
    fn test_priority_score_stablecoin_conversion_fallback() {
        ...
        // fee * 1M / 2M = fee / 2
        let score = registry.priority_score(2_000_000, false, 2);
    ...
    fn test_get_call_price_fallback() {
        let registry = FeeCurrencyRegistry::new();
        let price = registry.get_call_price();
