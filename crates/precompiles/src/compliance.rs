...
use crate::{
    decode_address, decode_u64, decode_u8, encode_u8, ok_empty, slot_compliance, u256_to_address,
    u256_to_u8, u8_to_u256,
};
...
impl CompliancePrecompile {
    fn update_compliance(&self, input: &[u8], msg_sender: alloy_primitives::Address) -> crate::PrecompileResult {
        ...
        if StorageCtx::is_static() {
            return Err(revm_precompile::PrecompileError::Other("static call cannot mutate state".into()));
        }
    ...
    fn check_compliance(&self, input: &[u8]) -> crate::PrecompileResult {
        ...
        // Derive policy_id from asset metadata
        let policy_slot = crate::storage::storage_slot(&[&asset_id.to_be_bytes()[..], b"compliance"]);
        let policy_id = StorageCtx::sload(crate::ASSET_ADDRESS, policy_slot)
            .map(u256_to_u8)
            .unwrap_or(0);
