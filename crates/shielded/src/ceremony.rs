...
impl ProductionKeys {
    ...
    /// Get the proving key for a specific circuit (if loaded).
    pub fn pk(&self, circuit: CircuitType) -> Option<&ProvingKey<Bn254>> {
        match circuit {
            CircuitType::Transfer => self.pk.as_ref().map(|p| &p.transfer),
            CircuitType::Deposit => self.pk.as_ref().map(|p| &p.deposit),
            CircuitType::Withdraw => self.pk.as_ref().map(|p| &p.withdraw),
        }
    }
}
...
// Re-export Fr for convenience
pub use ark_bn254::Fr as FrElement;
