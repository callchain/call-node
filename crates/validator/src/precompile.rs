...
impl ValidatorPrecompile {
    fn stake(&self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        dispatch::mutate_void::<IProtocolValidator::stakeCall, _>(calldata, 20000, |call| {
    ...
    fn unstake(&self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
    ...
    fn claim_unbonded(&self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        dispatch::mutate_void::<IProtocolValidator::claimUnbondedCall, _>(calldata, 15000, |call| {
            let caller = require_caller(msg_sender)?;
            let backend = JournalBackend;
            let mut validator_store = ValidatorStorage::new(backend);
            let mut asset_store = AssetStorage::new(backend);
    ...
    fn get_validator_stake(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolValidator::getValidatorStakeCall, _, _>(calldata, 1000, |call| {
            let store = ValidatorStorage::new(JournalBackend);
            Ok(store.read_stake(call.validator))
        })
    }

    fn get_validator_status(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolValidator::getValidatorStatusCall, _, _>(calldata, 1000, |call| {
            let store = ValidatorStorage::new(JournalBackend);
            Ok(U256::from(store.read_status(call.validator)))
        })
    }

    fn get_validator_pubkey(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolValidator::getValidatorPubkeyCall, _, _>(calldata, 1000, |call| {
            let store = ValidatorStorage::new(JournalBackend);
            Ok(store.read_pubkey(call.validator))
        })
    }

    fn get_unbond_height(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolValidator::getUnbondHeightCall, _, _>(calldata, 1000, |call| {
            let store = ValidatorStorage::new(JournalBackend);
            Ok(store.read_unbond_height(call.validator))
        })
    }

    fn get_validator_by_index(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolValidator::getValidatorByIndexCall, _, _>(calldata, 1000, |call| {
            let store = ValidatorStorage::new(JournalBackend);
...
impl call_precompile::StatefulPrecompile for ValidatorPrecompile {
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        match selector {
            IProtocolValidator::stakeCall::SELECTOR => self.stake(calldata, msg_sender),
            IProtocolValidator::unstakeCall::SELECTOR => self.unstake(calldata, msg_sender),
            IProtocolValidator::claimUnbondedCall::SELECTOR => {
                self.claim_unbonded(calldata, msg_sender)
            }
            IProtocolValidator::getValidatorStakeCall::SELECTOR => self.get_validator_stake(calldata),
            IProtocolValidator::getValidatorStatusCall::SELECTOR => {
                self.get_validator_status(calldata)
            }
            IProtocolValidator::getValidatorPubkeyCall::SELECTOR => {
                self.get_validator_pubkey(calldata)
            }
            IProtocolValidator::getUnbondHeightCall::SELECTOR => {
                self.get_unbond_height(calldata)
            }
            IProtocolValidator::getValidatorByIndexCall::SELECTOR => {
                self.get_validator_by_index(calldata)
            }
...
mod tests {
    ...
    fn test_validator_precompile_stake_and_get() {
        ...
        call_precompile::storage::StorageCtx::enter(&mut provider, || {
            ...
            let input = IProtocolValidator::getValidatorStakeCall {
                validator: sender,
            }
            .abi_encode();
            ...
            let input = IProtocolValidator::getValidatorStatusCall {
                validator: sender,
            }
            .abi_encode();
    ...
    fn test_validator_precompile_read_methods() {
        ...
        call_precompile::storage::StorageCtx::enter(&mut provider, || {
            ...
            let input = IProtocolValidator::getValidatorPubkeyCall {
                validator: sender,
            }
            .abi_encode();
            let result = precompile.call(&input, Address::ZERO).unwrap();
            ...
            let input = IProtocolValidator::getUnbondHeightCall {
                validator: sender,
            }
            .abi_encode();
            let result = precompile.call(&input, Address::ZERO).unwrap();
