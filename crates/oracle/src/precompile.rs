...
impl OraclePrecompile {
    fn get_price(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolOracle::getPriceCall, _, _>(calldata, 1000, |call| {
            let store = OracleStorage::new(JournalBackend);
            Ok(store.read_price(call.assetId))
        })
    }

    fn get_twap(&self, calldata: &[u8]) -> PrecompileResult {
        dispatch::view::<IProtocolOracle::getTWAPCall, _, _>(calldata, 1000, |call| {
            let store = OracleStorage::new(JournalBackend);
    ...
    fn submit_price(&self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        dispatch::mutate_void::<IProtocolOracle::submitPriceCall, _>(calldata, 30000, |call| {
...
impl call_precompiles::StatefulPrecompile for OraclePrecompile {
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        match selector {
            IProtocolOracle::getPriceCall::SELECTOR => self.get_price(calldata),
...
mod tests {
    ...
    fn test_oracle_precompile_submit_and_get() {
        ...
        call_precompiles::storage::StorageCtx::enter(&mut provider, || {
            ...
            let price = u256_to_u128(U256::from_be_bytes::<32>(result.bytes.as_ref().try_into().unwrap()));
    ...
    fn test_oracle_precompile_twap_and_stale() {
        ...
        call_precompiles::storage::StorageCtx::enter(&mut provider, || {
            ...
            let result = precompile.call(&input, Address::ZERO).unwrap();
            let twap = u256_to_u128(U256::from_be_bytes::<32>(result.bytes.as_ref().try_into().unwrap()));
