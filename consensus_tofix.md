...
### 12.4 Full Node 缺少 `allow_private_ips`（High）
...
### 12.5 Full Node 不在 Validator 的 bootstrap 列表中（High）
...
Full node 可以主动连 validator，但 validator 永远不会回 dial 它们。如果 full node 的监听地址不可达或存在 NAT 问题，单向连接可能失败。

**修复：**
把所有 validator 的 `bootstrap_peers` 中加上 `172.28.0.15:51235`（node5）和 `172.28.0.16:51235`（node6）。同时给 node5/6 配置固定的 `identity_key`，让 validator 能识别其持久 peer ID。
...
### 12.7 节点 1 Validator Set 分叉（Medium）
...
| P1 | Full node 缺少 `allow_private_ips` | `devnet/configs/node5.toml`, `node6.toml` |
| P1 | Validator bootstrap 列表补全 full node | `devnet/configs/node1-4.toml` |
| P2 | `eth_getBlockByNumber` 从磁盘加载、返回真实 hash ✅ 已改，待验证 | `crates/rpc/src/standard.rs` |
| P2 | propose 阶段隔离状态修改，防止 validator set分叉 | `crates/node/src/lib.rs` 以及多个 crate | ✅ 已改，编译通过，125/126 测试通过（1 个 logging 测试为历史问题） |

 Callchain 状态隔离 + State Root 校验方案

     Context

     Callchain 的 BFT propose/verify 阶段直接对共享状态执行 block.execute()，导致状态在共识达成前就被修改。这是节点 1 validator set 分叉的根因（节点 1 propose 了一个 stake 交易，P2P
     广播被 drop，但本地状态已被修改）。

     参考实现的状态隔离原则：
     - arc-node: propose/verify 通过 Engine API 生成/验证 payload，不修改 CL 共识状态；只在 Decided (finalize) 后通过 forkchoice_updated 提交。
     - tempo: propose/verify 在克隆的 Inner<TState> 和临时的 reth_revm::State 上运行，丢弃后即销毁；Executor Actor 顺序处理 finalize。
     ...
     设计一个短期可实施（修改范围小）+ 长期正确（state root 校验）的方案，修复 propose/verify/finalize 的状态隔离问题。
     ...
     - propose: 在状态克隆上执行 block，只缓存 block + execution result，不修改共享状态
     - verify: 在状态克隆上验证 block（或信任本地缓存），不修改共享状态
     ...
     ValidatorStateManager(crates/consensus/src/validator.rs:78)、EvmState(crates/evm/src/state.rs:93)、BridgeStateManager、ShieldedState、ComplianceEngine、FeeParams 已实现了
     Clone，无需修改。

     1.2 修改 propose handler (crates/node/src/lib.rs:2388-2467)
     ...
     - oracle.advance_period() 和 distribute_rewards() 等副作用从 propose 阶段移除，移到 finalize 阶段

     1.3 修改 verify handler (crates/node/src/lib.rs:2494-2544)
     ...
     Some((digest, reply_tx)) = verify_rx.recv() => {
         ...
         let valid = if let Some(block) = block {
             ...
             match block.execute(
                 &mut balances, &mut registry, &mut compliance,
                 &mut bridge_state, &mut shielded_state, &mut fee_params,
                 block.header.height, &mut evm_state, Some(&mut oracle), None,
                 Some(&mut agent_balances), Some(&mut agent_registry),
                 None, None, None, None,
                 Some(&mut validator_state),
                 None,
     ...
     1.4 修改 finalize handler (crates/node/src/lib.rs:2546-2758)

     finalize 是唯一修改共享状态的阶段。当前代码有两种路径：
     1. cache hit (execution_results.remove): 直接用 cached result，不重新执行
     2. cache miss: 重新执行 block，修改共享状态

     问题分析： 路径 1 不执行 block，意味着共享状态从未被更新（但以前 propose 阶段已偷偷修改了，所以看起来正常）。路径 2 是正确的行为，但只在 cache miss 时触发。
     ...
     Some(info) = finalize_rx.recv() => {
         ...
         if let Some(mut block) = block {
             ...
             let result = match block.execute(
                 &mut balances, &mut registry, &mut compliance,
                 &mut bridge_state, &mut shielded_state, &mut fee_params,
                 height, &mut evm_state, Some(&mut *oracle),
                 None, Some(&mut *agent_balances), Some(&mut *agent_registry),
                 None, None, None, None,
             ...
             if is_oracle_boundary {
                 ...
                 let contributions = state.oracle.write().unwrap().distribute_rewards();
                 // ...（保持现有逻辑）
             ...
             // 注意：不再使用 execution_results 中的 cached result
             // execution_results 现在仅供 verify 阶段参考
         }
     }
     ...
     1.5 修改 sync handler (crates/node/src/lib.rs:2984-3053)
     ...
     fn apply_synced_blocks(
         ...
         let roots_valid = {
             ...
             match block.execute(
                 &mut balances, &mut registry, &mut compliance,
                 &mut bridge_state, &mut shielded_state, &mut fee_params,
                 block_height, &mut evm_state, None, None,
                 Some(&mut agent_balances), Some(&mut agent_registry),
                 None, None, None, None,
                 Some(&mut validator_state),
         ...
         let execute_result = {
             let mut balances = state.balance_state.write().unwrap();
             // ...（保持现有逻辑）...
             block.execute(&mut balances, ..., Some(&mut *state.validator_state.write().unwrap()), ...)
         };

         match execute_result {
             Ok(result) => {
                 block.finalize(&result);
                 persist_block(data_dir, block_height, &block);
                 consensus.write().unwrap().commit_block(&block, &result);
     ...
     1.6 添加 commit_block 高度防重放 (crates/consensus/src/simplex.rs:229)
     ...
     Phase 1 已经在 verify 和 sync 阶段引入了 state root 校验。Phase 2 进一步优化，使 verify 阶段不需要完整重新执行 block。

     2.1 统一 State Root

     当前 BlockHeader 有四个独立的 root 字段。可以考虑添加一个统一的 state_root：
     ...
     │              文件               │                   修改内容                    │ 优先级 │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/protocol/src/registry.rs │ AssetRegistry 加 Clone                        │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/agent/src/balances.rs    │ AgentBalances 加 Clone                        │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/agent/src/registry.rs    │ AgentRegistry 手动实现 Clone                  │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/consensus/src/fork.rs    │ ForkManager / RollbackRecord 加 Clone         │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/oracle/src/lib.rs        │ OracleManager 加 Clone                        │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/node/src/lib.rs          │ propose handler 改为克隆执行                  │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/node/src/lib.rs          │ verify handler 改为克隆 + state root 校验     │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/node/src/lib.rs          │ finalize handler 改为共享状态执行 + root 校验 │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/node/src/lib.rs          │ sync handler 添加 state root 验证             │ P0 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/consensus/src/simplex.rs │ commit_block 添加高度防重放                   │ P1 ✅  │
     ├─────────────────────────────────┼───────────────────────────────────────────────┼────────┤
     │ crates/consensus/src/block.rs   │ 添加统一 state_root 字段到 BlockHeader        │ P2 ✅  │
     └─────────────────────────────────┴───────────────────────────────────────────────┴────────┘

     ---
     验证计划

     1. 编译检查: cargo check -p call-node -p call-consensus
     2. 单元测试: cargo test -p call-node -p call-consensus
     3. 状态隔离测试: 在 test_consensus_block_production.rs 中添加测试：
       - propose 后检查共享状态未被修改
       - finalize 后检查共享状态已被修改
       - verify 后检查共享状态未被修改
     4. State Root 校验测试: 修改一个 state root 后验证 block 被拒绝
     5. Devnet 验证: 运行 6 节点 devnet，确认：
       - 所有节点高度一致
       - eth_getBlockByNumber 返回正确的 state roots
       - epoch rotation 后状态一致