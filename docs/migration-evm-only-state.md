...
| Phase 7 — Persistence | 🔄 Partial | `LoadedState` simplified to `evm_state` + `agent_nonces` + `governance` + `fee_params`. Protocol-state save/load removed. Oracle and Governance sidecars still persisted. Some dead DB table types remain in `reth_db.rs`. |
...
| Phase 10 — Cleanup | 🔄 Partial | `AccountState`, `AssetRegistry`, `ComplianceEngine`, `BridgeStateManager`, `ValidatorStateManager`, `AgentRegistry`, `AgentBalances`, `ShieldedState` still exist in source crates but are **unused** in `RpcState` / node paths. `OracleManager` and `GovernanceManager` remain as sidecars (not in `RpcState`). |
