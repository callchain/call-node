...
Callchain's privacy layer is implemented via the `ShieldedPrecompile` at address `0x202` (`crates/shielded/`). It provides a **UTXO-based shielded pool** with Halo2 zk-SNARK verification over Pasta curves.
...
|-----------|---------------|
| **Proof System** | Halo2 on Pasta curves (`halo2_proofs` 0.3) |
...
| **Hash Function** | Poseidon (plain + `halo2_gadgets::poseidon`) |
...
| **Performance** | Proof generation: 2-5s; Verification: ~5-10ms; Proof size: ~5-10KB |
...
| **Recursive SNARKs** | Recursive SNARKs to compress/aggregate proofs so privacy costs the same as transparency | Independent Halo2 proofs per transaction; no recursive aggregation yet; total verification cost scales linearly with tx count | **Gap**: No proof aggregation or recursion |
...
**Current State**: Each `transfer()` carries an independent Halo2 proof. Verifying 50 shielded tx in a block costs ~250-500ms (50 x 5-10ms).

**Vitalik's Position**: Recursive SNARKs allow many private payments to be verified at the cost of one.

**Potential Directions**:
- **Batch verification**: Verify multiple Halo2 proofs with shared randomness.
...
| Alignment | Evidence |
|-----------|----------|
| **Real ZK Proofs** | Full Halo2 circuits (Deposit/Withdraw/Transfer) with `halo2_proofs`; 120+ passing tests; no mock stubs in production path |
| **EVM-Native Verification** | Verified in native Rust precompile (`0x202`); no EVM precompile for Pasta curves |
...
| **Production Key Infrastructure** | Universal params (`Params::new(k)`), no trusted setup; key rotation via governance |
| **Remote Prover Service** | `call-prover` HTTP server offloads proof generation from client devices |
| **Performance** | 5-10ms verification, ~5-10KB proof size, 50 tx/block -- suitable for retail payment throughput |
...
The core cryptographic infrastructure -- Halo2 circuits, Poseidon hashing, nullifier sets, Merkle trees, viewing keys, and compliance modes -- is fully implemented and tested. The gap is not in engineering but in **protocol defaults and integrations**:
...
- `crates/shielded/src/circuit_*.rs` -- Halo2 circuit implementations
