...
| 2 | **Oracle round missing from BFT propose** | **High** | The old `block_production_loop` requested oracle price submissions from validators at `ORACLE_UPDATE_INTERVAL` boundaries before building the block. `bft_event_loop::propose` skips this entirely, so oracle prices will not advance under BFT consensus. Fix: inject oracle price request → sleep → collect flow into the propose handler. |
| 3 | **Dynamic validator set not propagated** | **High** | The `Ed25519Scheme` participant set is snapshotted once at engine startup. If validators stake or unbond while the node is running, the BFT engine's view of the validator set becomes stale. Fix: restart the engine when the validator set changes, or implement participant hot-swap. |
| 4 | **BFT journal state not persisted** | **Medium** | `PersistedConsensusState` saves height/round/hash, but the `simplex::Engine` maintains an internal journal (notarizations, finalizations, activity buffer) that is lost on restart. After a crash the node may rejoin at the wrong view. Fix: wire the engine's journal to disk via `commonware_runtime::buffer::paged`. |
